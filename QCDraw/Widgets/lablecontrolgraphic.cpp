#include "lablecontrolgraphic.h"
#include <QPainter>
#include <QHBoxLayout>

const QString LableControlGraphic::_devName = CodeConvertor::fromLocal("±êÇ©");

LableControlGraphic::LableControlGraphic(QWidget* parent) :
    WidgetGraphicBase(parent)
{
    this->setType(Graphic::GRAPHIC_CONTROL_LABLE);
    _lable = new QLabel("±êÇ©",this);
    _lable->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
    this->setCurWidget(_lable);
    //this->resize(_table->width() + 12,_table->height() + 12);
}

void LableControlGraphic::loadXML(rapidxml::xml_node<char> *node)
{
...
void LableControlGraphic::restoreGraphic(QDataStream& dataStream)
{
    dataStream >> _name;
    dataStream >> _id;
    QRect rect;
...
void LableControlGraphic::saveXML(rapidxml::xml_node<char> *node, rapidxml::xml_document<> &doc,Graphic* graphic)
{
...
void LableControlGraphic::saveGraphic(QDataStream& dataStream) const
{
    dataStream << _type;
    dataStream << _name;
    dataStream << _id;
