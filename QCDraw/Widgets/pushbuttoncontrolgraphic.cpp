#include "pushbuttoncontrolgraphic.h"
#include <QPainter>
#include <QHBoxLayout>

const QString PushButtonControlGraphic::_devName = CodeConvertor::fromLocal("±êÇ©");

PushButtonControlGraphic::PushButtonControlGraphic(QWidget* parent) :
    WidgetGraphicBase(parent)
{
    this->setType(Graphic::GRAPHIC_CONTROL_PUSHBUTTON);
    _pushButton = new QPushButton(this);
    _pushButton->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Expanding);
    this->setCurWidget(_pushButton);
    //this->resize(_table->width() + 12,_table->height() + 12);
}

void PushButtonControlGraphic::loadXML(rapidxml::xml_node<char> *node)
{
...
void PushButtonControlGraphic::restoreGraphic(QDataStream& dataStream)
{
    dataStream >> _name;
    dataStream >> _id;
    QRect rect;
...
void PushButtonControlGraphic::saveXML(rapidxml::xml_node<char> *node, rapidxml::xml_document<> &doc,Graphic* graphic)
{
...
void PushButtonControlGraphic::saveGraphic(QDataStream& dataStream) const
{
    dataStream << _type;
    dataStream << _name;
    dataStream << _id;
