#include "muticurvechartcontrolgraphic.h"
#include <QPainter>
#include <QHBoxLayout>

const QString MutiCurveChartControlGraphic::_devName = CodeConvertor::fromLocal("ÕÛÏßÍ¼");

MutiCurveChartControlGraphic::MutiCurveChartControlGraphic(QWidget* parent) :
    WidgetGraphicBase(parent)
{
    this->setType(Graphic::GRAPHIC_CONTROL_MUTICURVECHART);
    _mutiCurveChart = new MutiCurveChart(this);
    _mutiCurveChart->setMinimumWidth(420);
    _mutiCurveChart->setMinimumHeight(120);
    this->setCurWidget(_mutiCurveChart);
    //this->resize(_table->width() + 12,_table->height() + 12);
}

void MutiCurveChartControlGraphic::loadXML(rapidxml::xml_node<char> *node)
{
...
void MutiCurveChartControlGraphic::restoreGraphic(QDataStream& dataStream)
{
    dataStream >> _name;
    dataStream >> _id;
    QRect rect;
...
void MutiCurveChartControlGraphic::saveXML(rapidxml::xml_node<char> *node, rapidxml::xml_document<> &doc,Graphic* graphic)
{
...
void MutiCurveChartControlGraphic::saveGraphic(QDataStream& dataStream) const
{
    dataStream << _type;
    dataStream << _name;
    dataStream << _id;
